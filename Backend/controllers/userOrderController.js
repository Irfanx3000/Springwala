...
exports.placeOrder = async (req, res) => {
  try {
    ...
    // 5. Post-Order Actions (Stock, Stats & Cart Cleanup)
    for (const item of validatedItems) {
      await Product.findByIdAndUpdate(item.product, {
        $inc: { stock: -item.quantity, totalSold: item.quantity },
      });
    ...
    // Clear server-side cart after successful order
    try {
      const Cart = require('../models/Cart');
      ...
      console.log('[ORDER] Server cart cleared for user:', req.user._id);
    } catch (cartErr) {
      console.warn('[ORDER] Could not clear server cart (non-critical):', cartErr.message);
    }
